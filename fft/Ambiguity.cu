...
//Warning 
// ERROR codes are in header file!
// The settings are almost save for RTX3060 because this library operates mostly on the memory. RTX 3060 has a limit of 6GB and tables for FFT are Col * Nth
//minimum Column (Col) = 10 if external proggram ask for less columns than the dll will return error
//Maximum Column (Col) = 512
...
int  Initialize(unsigned int BufferSize, unsigned int col, unsigned int row, float doopler_shift, short* name)
{
	...
	blocksPerGrid = Nth / threadsPerBlock;


	//Protect ageinst wrong column number
	if (col < 10 || col>1024) return CUDA_RUNNING;
	//Memory limit on card 6GB but we need other arrays
	if(col*Nth> 5 * GB) return CUDA_RUNNING;
	Col = col;
	Row = row;
	ColRow = Col * Row;
	...
	//The most memory consuming part
	//In this way a lot of time for syncronisations is saved
	if (cufftPlan1d(&plan2, (int)Nth, CUFFT_C2C, Col) != CUFFT_SUCCESS) {
		return CUDA_FFT_PLAN1D_CREATE_ERROR;
	}
...
int  Run(int* Data_In0, int* Data_In1, float* Data_Out, float amplification, float doppler_zoom, int time_shift, bool mode, short scale_type, bool remove_symetrics)
{
	...
	//Protections
	//Memory limit on card 6GB but we need other arrays this is why is 5GB
	if (Col * Nth > 5 * GB) 
		return CUDA_RUNNING;
...
#pragma omp parallel for
	...
	//Input: Cuda_ColRow
	//Output: Cuda_bufW
	err = Magnitude(shift_cor, scale_type);
	if (err < CUDA_OK)
		return err;
	...
	//Copy from device to machine all the results in one shot (it saves a lot of time) Data_Out contains all shifted lines (the rough image)
...
{
	//Backward fft
	if (cufftExecC2C(plan2, Cuda_bufZ, Cuda_bufW, CUFFT_INVERSE) != CUFFT_SUCCESS) 
	{
		return CUDA_FFT_EXECUTE_ERROR;
	}
...
int  Release()
{
	...
	if ((su = cufftDestroy(plan2)) != cudaSuccess) {
		//fprintf(stderr, "cufftDestroy failed!");
		return CUDA_CUFFT_DESTROY_ERROR - su;
	}
...
{
	if(Row<1024)
	MagnitudeCUDA << <1, Row>> > (Cuda_bufW, Cuda_ColRow, Row, Col, shift, scale_Type, Nth);
	else
	{
		int blocks = Row / 256;
		MagnitudeCUDA << <blocks, 256 >> > (Cuda_bufW, Cuda_ColRow, Row, Col, shift, scale_Type, Nth);
	}
...
{
	CorelateShiftCUDA << <blocksPerGrid, threadsPerBlock >> > (Cuda_bufX, Cuda_bufY, Cuda_bufZ, Doppler_zoom_Col, Col, Nth);
	...
	//Do not need to be synchronise here!
	//int err = Synchronise();
	//if (err < CUDA_OK)
	//	return err;
 ...
 *
 */
__global__ void CorelateShiftCUDA(cufftComplex* BufX, cufftComplex* BufY, cufftComplex* BufZ, float shift, size_t cudaCol,  size_t cudaN) {

	size_t i = blockDim.x * blockIdx.x + threadIdx.x;
 
	for (int n = 0; n < cudaCol; n++)
	{
		size_t j = i + n * shift;
		size_t k = i + n * cudaN;
 
			//Just rotate from end to the beginning and convolute
			if (j < cudaN) //protection
			{
 ...
 */
__global__ void ShiftCUDA(cufftComplex* In, cufftComplex* Out, size_t shift, size_t cudaN) {

	size_t i = blockDim.x * blockIdx.x + threadIdx.x;
	size_t j = i + shift;

	//Just rotate from end to the beginning
	if (j < cudaN)  //To the end
		Out[i] = In[j];
	else
	{
		if (i < cudaN) //from the beginning
			Out[i] = In[j - cudaN];
	}
 ...
 */
__global__ void CopyShiftCUDA(cufftComplex* Buf0, cufftComplex* BufX, cufftComplex* BufY, size_t shift, size_t cudaN) {
 ...
 * Copy Inp1 to Inp2
 */
__global__ void CopyCUDA(cufftComplex* Inp1, cufftComplex* Inp2, size_t cudaN) {
	size_t i = blockDim.x * blockIdx.x + threadIdx.x;
 ...
 */
__global__ void CorelateCUDA(cufftComplex* InpX, cufftComplex* InpY, cufftComplex* Out, size_t cudaN) {
	size_t i = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void MagnitudeCUDA(cufftComplex* Inp, float* Out, int cuda_row, int cuda_col, int cuda_shift, short scale_type, int cudaN)
{
	...
	for (int n = 0; n < cuda_col; n++)
	{
		int j = i + cuda_shift + n * cudaN;
		int k = i + n * cuda_row;
